<x-dashboard-layout title="إضافة قسم">
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold tracking-tight text-[#f0ece3]">{{ __('إضافة قسم جديد') }}</h1>
            <p class="mt-1 text-sm text-[#9a9690]">{{ __('أدخل تفاصيل القسم لإضافته لتصنيفات قائمة الطعام') }}</p>
        </div>
        ...
        <form action="{{ route('categories.store') }}" method="POST" class="p-6">
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name') }}" class="field-input mt-1" placeholder="{{ __('مثال: المشروبات الساخنة') }}" required autofocus>
                    @error('name')
                ...
                <div>
                    ...
                    <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', 0) }}" min="0" class="field-input mt-1">
